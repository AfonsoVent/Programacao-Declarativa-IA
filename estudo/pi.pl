...
% pesquisa a -> pesquisa A*
pesquisa(a,E,S):- pesquisa_a(E,S).
pesquisa_a([no(E,Pai,Op,C,HC,P)|_],no(E,Pai,Op,C,HC,P)):- estado_final(E).
pesquisa_a([E|R],Sol):- 
    expandea(E,Lseg), %esc(E),
    insere_ord(Lseg,R,Resto),
    pesquisa_a(Resto,Sol).
expandea(no(E,Pai,Op,C,HC,P),L):- findall(no(En, no(E,Pai,Op,C,HC,P),Opn,Cnn,HCnn,P1), (op(E,Opn,En,Cn),P1 is P+1, Cnn is Cn+C, h(En,H),  HCnn is Cnn+H), L).

% pesquisa g -> pesquisa gready 
pesquisa(g,E,S):- pesquisa_g(E,S).
pesquisa_g([no(E,Pai,Op,C,HC,P)|_],no(E,Pai,Op,C,HC,P)):- estado_final(E).
pesquisa_g([E|R],Sol):- expandeg(E,Lseg), %esc(E),
    insere_ord(Lseg,R,Resto),
    pesquisa_g(Resto,Sol).

expandeg(no(E,Pai,Op,C,HC,P),L):- findall(no(En, no(E,Pai,Op,C,HC,P),Opn,Cnn,H,P1), (op(E,Opn,En,Cn),P1 is P+1, Cnn is Cn+C, h(En,H)), L).
...
escreve_seq_solucao(no(E,Pai,Op,Custo,_HC,Prof)):- write(custo(Custo)),nl, write(profundidade(Prof)),nl, escreve_seq_accoes(no(E,Pai,Op,_,_,_)).
escreve_seq_accoes([]).
escreve_seq_accoes(no(E,Pai,Op,_,_,_)):- escreve_seq_accoes(Pai), write(e(Op,E)),nl.
