...
quantos:- 
    ...
    nosE(NE),
    nl, 
    write(nos(visitados(Ns),lista(NL),expandidos(NE))),
    nl.
...
expande(no(E,Pai,Op,C,P),L):-
    findall(no(En,no(E,Pai,Op,C,P),Opn,Cnn,P1), (op(E,Opn,En,Cn),P1 is P+1, Cnn is Cn+C,incE(1)), L).
...
expandePl(no(E,Pai,Op,C,P),L,_):- findall(no(En,no(E,Pai,Op,C,P),Opn,Cnn,P1), (op(E,Opn,En,Cn),P1 is P+1, Cnn is Cn+C, incE(1)),L).
