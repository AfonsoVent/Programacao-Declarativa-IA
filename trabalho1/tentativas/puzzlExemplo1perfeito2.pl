...
% Andar para cima
op(((X, Y), ObjA, ObjB, Visited), 'up', ((X, Y1), NewObjA, NewObjB, NewVisited), 1):- 
    Y1 is Y + 1,
    limits(X, Y1),
    \+ wall(X, Y1),
    \+ checkVisited(ObjA, ObjB, X, Y1, Visited),
    updateVisited(ObjA, ObjB, X, Y1, Visited, NewVisited),
    take(X, Y1, ObjA, ObjB, NewObjA, NewObjB).

% Andar para X direita
op(((X, Y), ObjA, ObjB, Visited), 'right', ((X1, Y), NewObjA, NewObjB, NewVisited), 1):- 
    X1 is X + 1,
    limits(X1, Y),
    \+ wall(X1, Y),
    \+ checkVisited(ObjA, ObjB, X1, Y, Visited),
    updateVisited(ObjA, ObjB, X1, Y, Visited, NewVisited),
    take(X1, Y, ObjA, ObjB, NewObjA, NewObjB).

% Andar para baixo
op(((X, Y), ObjA, ObjB, Visited), 'down', ((X, Y1), NewObjA, NewObjB, NewVisited), 1):- 
    Y1 is Y - 1,
    limits(X, Y1),
    \+ wall(X, Y1),
    \+ checkVisited(ObjA, ObjB, X, Y1, Visited),
...
op(((X, Y), ObjA, ObjB, Visited), 'left', ((X1, Y), NewObjA, NewObjB, NewVisited), 1):- 
    X1 is X - 1,
    limits(X1, Y),
    \+ wall(X1, Y),
    \+ checkVisited(ObjA, ObjB, X1, Y, Visited),
...
take(X, Y, false, ObjB, true, ObjB) :- takeA(X, Y).
take(X, Y, ObjA, false, ObjA, true) :- takeB(X, Y).
take(_, _, ObjA, ObjB, ObjA, ObjB).
